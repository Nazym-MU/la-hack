...
For each room, output only the marblePrompt itself: 3 to 6 dense sentences of spatial
detail, nothing else. No labels, no field names, no commentary before or after it.

Since one batch of input can produce several rooms, separate each room's marblePrompt
with a blank line so they stay distinct, still with no extra text, headers, or numbering
around them.